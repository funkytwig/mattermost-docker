...
echo
echo $cut_from $cut_to
echo

days=`ssh $backup_login \
  "find $backup_path -name \"*_F_*\" -maxdepth 1 -mmin +1500|rev|cut -c$cut_from-$cut_to|rev|sort|uniq"`

for day in $days; do
  oldest_file=`ssh $backup_login "ls -td ${backup_path}/*${day}* | sort |head -1"`
  rename_to=`echo $oldest_file| sed "s/_F_/_D_/g"
  echo "mv $oldest_file $rename_to"
done
